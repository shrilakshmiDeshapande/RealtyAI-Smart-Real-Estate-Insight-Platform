...
# --- MODULE 3: Trend Forecasting ---
    ...
    st.info("Select region and forecast horizon.")
    ...
    if st.button("🔍 Preview Historical Data", key="preview_btn", use_container_width=True):
        try:
            with open(PROPHET_MODELS[region], "r") as f:
                prophet_model = model_from_json(f.read())
            
            # Get last 10 historical points
            history_df = prophet_model.history[['ds', 'y']].tail(10).copy()
            history_df.columns = ['Date', 'Historical_ZHVI']
            history_df['Region'] = region
            history_df = history_df[['Region', 'Date', 'Historical_ZHVI']]  # Reorder
            history_df['Historical_ZHVI'] = history_df['Historical_ZHVI'].round().astype(int)
            history_df['Date'] = history_df['Date'].dt.strftime('%Y-%m-%d')
            
            st.subheader("🔍 Historical Data Preview (Last 10 Months)")
            st.dataframe(history_df, use_container_width=True)
        except Exception as e:
            st.warning(f"⚠️ Could not load historical preview: {e}")
    ...
    if st.button("🔮 Generate Forecast", key="trend_btn", use_container_width=True):
        try:
            ...
            # ✅ OUTPUT: Date, Region, Lower, Value, Upper
            last_train_date = prophet_model.history['ds'].max()
            forecast_future = forecast[forecast['ds'] > last_train_date].copy()
            forecast_table = forecast_future[['ds', 'yhat_lower', 'yhat', 'yhat_upper']].copy()
            forecast_table.columns = ['Date', 'Lower_Bound', 'Predicted_ZHVI', 'Upper_Bound']
            forecast_table.insert(1, 'Region', region)  # Add region column
            ...
            st.download_button(
                ...
                f"forecast_{region}_{years}y.csv",
                "text/csv",
                use_container_width=True
            )
        except Exception as e:
